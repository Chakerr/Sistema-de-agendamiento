...
public interface ReservasRepository extends JpaRepository<Reservas,Integer> {
    ...
    @Query("SELECT r.estudiantesList FROM Reservas r WHERE r.id = :idReserva")
    List<Estudiantes> findEstudiantesByReservaId(@Param("idReserva") Integer idReserva);

    @Query("SELECT r FROM Reservas r JOIN r.estudiantesList e WHERE r.id = :reservaId AND e.id_codigo = :codigoEstudiante")
    Reservas findByReservaIdAndEstudianteCodigo(@Param("reservaId") Integer reservaId, @Param("codigoEstudiante") Integer codigoEstudiante);
}
